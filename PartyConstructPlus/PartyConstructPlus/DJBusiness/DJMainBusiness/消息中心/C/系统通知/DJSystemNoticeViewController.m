//
//  DJSystemNoticeViewController.m
//  PartyConstructPlus
//
...
static  NSInteger  indexRow;

@interface DJSystemNoticeViewController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation DJSystemNoticeViewController {
...
- (void)viewDidLoad {
    ...
    __weak  DJSystemNoticeViewController *serlWeak = self;
    self.returnClickBlock = ^(NSString *strValue) {
        if ([serlWeak.animationType isEqualToString:@"present"]) {
            [serlWeak.navigationController dismissViewControllerAnimated:YES completion:nil];
        }else {
...
- (void)createTableView {
    ...
    self.dataEmptyView.promptLabel.text = @"暂无系统通知";
    self.dataEmptyView.dividerLabel.alpha =0;
    self.dataEmptyView.emptyImageView.image = [UIImage imageNamed:@"DJ_xttz_empty"];
...
-(void)RequestTaskNotificationList {
    ...
    [URL_Dic setObject:@"system" forKey:@"msgType"];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [cell.iconBtn setImage:[UIImage imageNamed:@"DJSystemNotice"] forState:(UIControlStateNormal)];
