...
@interface DJSJFXOrgChooseView ()<UITableViewDelegate, UITableViewDataSource>

@property (strong, nonatomic) UITableView *tableview;

@end
@implementation DJSJFXOrgChooseView
...
- (UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    LowerOrgModel *model = self.dataArr[indexPath.row];
    cell.textLabel.text = model.name;
    if ([model.isSelectAll isEqualToString:@"1"]) {
        cell.textLabel.textColor = kColorRGB(253, 115, 77, 1);
    }else {
        cell.textLabel.textColor = kColorRGB(51, 51, 51, 1);
    }
    return cell;
}
- (void) tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    self.isConceal = NO;
    if ([_Delegate respondsToSelector:@selector(OrgChooseWithIndex:)]) {
        [self.Delegate OrgChooseWithIndex:indexPath.row];
    }
