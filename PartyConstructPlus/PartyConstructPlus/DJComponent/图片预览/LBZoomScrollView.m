...
- (void)reloadCellDataWithModel:(LBScrollViewStatusModel *)model andImage:(UIImage *)image andImageData:(NSData *)data{
    ...
    if (model.currentPageImage == NULL ||model.currentPageImage == nil) {
...
- (void)resetScrollViewStatusWithImage:(UIImage *)image {
    NSLog(@"image.size.height%f image.size.width%f self.width%f image%@", image.size.height, image.size.width, self.width, image);
    ...
    if (imageHeight / imageWidth > self.height / self.width) {
        
        self.imageView.height = floor(imageHeight / (imageWidth / self.width));
    }else {
        
        CGFloat height = imageHeight/ imageWidth * self.width;;
