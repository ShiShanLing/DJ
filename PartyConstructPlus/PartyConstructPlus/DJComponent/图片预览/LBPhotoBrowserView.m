...
- (void)loadImage {
    ...
    self.opreation = [[SDWebImageDownloader sharedDownloader] downloadImageWithURL:self.url options:0 progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (wself.loadImageCompletedBlock) {
                wself.loadImageCompletedBlock(wself, downloadedImage, nil, error, finished, self.url);
            }else {
                if (error) {
...
//    self.opreation = [[SDWebImageDownloader sharedDownloader] downloadImageWithURL:self.url options:0 progress:^(NSInteger receivedSize, NSInteger expectedSize) {
//
...
//            wself.opreation = nil;
//            if (wself.loadImageCompletedBlock) {
//                wself.loadImageCompletedBlock(wself, downloadedImage, nil, error, finished, imageURL);
//            }else {
//                if (error) {
//                    downloadedImage = [LBPhotoBrowserManager defaultManager].errorImage;
//                    LBPhotoBrowserLog(@"%@",error);
