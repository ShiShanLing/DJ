...
#import "SMKWebViewProgress.h"

NSString *SMKcompleteRPCURL = @"webviewprogressproxy:///complete";

const float SMKInitialProgressValue = 0.1f;
const float SMKInteractiveProgressValue = 0.5f;
const float SMKFinalProgressValue = 0.9f;

@implementation SMKWebViewProgress
{
...
- (void)startProgress
{
    if (_progress < SMKInitialProgressValue) {
        [self setProgress:SMKInitialProgressValue];
    }
}
...
- (void)incrementProgress
{
    ...
    float maxProgress = _interactive ? SMKFinalProgressValue : SMKInteractiveProgressValue;
...
{
    if ([request.URL.absoluteString isEqualToString:SMKcompleteRPCURL]) {
...
- (void)webViewDidFinishLoad:(UIWebView *)webView
{
    ...
    if (interactive) {
        _interactive = YES;
        NSString *waitForCompleteJS = [NSString stringWithFormat:@"window.addEventListener('load',function() { var iframe = document.createElement('iframe'); iframe.style.display = 'none'; iframe.src = '%@'; document.body.appendChild(iframe);  }, false);", SMKcompleteRPCURL];
