...
@interface SMKWebView () <UIWebViewDelegate, WKNavigationDelegate, WKUIDelegate>
...
- (UIProgressView *)progressView
{
    if(!_progressView)
    {
        ...
        self.progressView.tintColor = kColorRGB(228, 17, 53, 1);
...
- (void)setDelegate:(id<SMKWebViewDelegate>)delegate
{
    _delegate = delegate;
    if (_usingUIWebView) {
...
- (void)observeValueForKeyPath:(NSString*)keyPath ofObject:(id)object change:(NSDictionary*)change context:(void*)context {
    if (object == self.realWebView && [keyPath isEqualToString:@"estimatedProgress"]) {
        CGFloat newprogress = [[change objectForKey:NSKeyValueChangeNewKey] doubleValue];
        if (newprogress == 1) {
            self.progressView.hidden = YES;
            [self.progressView setProgress:0 animated:NO];
            self.webViewLoadingEndBlock();
        }else {
//            self.progressView.hidden = YES;
...
- (void)showHud:(NSString *)message title:(NSString *)title{
    newhud = [MBProgressHUD showHUDAddedTo:self animated:YES];
...
//关闭
-(void)hudDissmiss {
    [newhud hideAnimated:YES afterDelay:0.3];
