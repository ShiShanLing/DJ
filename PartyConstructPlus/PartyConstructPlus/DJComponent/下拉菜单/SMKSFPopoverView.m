...
#import "SMKSFPopoverView.h"
#import "SMKSFPopoverViewCell.h"
static float const kPopoverViewMargin = 10.f; ///< 边距
static float const kPopoverViewCellHeight = 45.f; ///< cell指定高度
static float const kPopoverViewArrowHeight = 9.f; ///< 箭头高度
...
@interface SMKSFPopoverView () <UITableViewDelegate, UITableViewDataSource>
...
#pragma mark - Data
@property (nonatomic, copy) NSArray<SMKSFPopoverAction *> *actions;
...
@implementation SMKSFPopoverView
...
- (void)setStyle:(PopoverViewStyle)style {
    _style = style;
    _tableView.separatorColor = [SMKSFPopoverViewCell bottomLineColorForStyle:_style];
...
- (void)initialize {
    ...
    _transparentView.sd_layout.leftSpaceToView(self, 0).topSpaceToView(self, 0).rightSpaceToView(self, 0).bottomSpaceToView(self, 0);
...
- (void)showToPoint:(CGPoint)toPoint {
    ...
    CGFloat currentH = _actions.count *kFit(kPopoverViewCellHeight) +kPopoverViewArrowHeight;
...
- (CGFloat)calculateMaxWidth {
    ...
    UIFont *titleFont = [SMKSFPopoverViewCell titleFont];
    for (SMKSFPopoverAction *action in _actions) {
        imageWidth = 0.f;
        titleLeftEdge = 0.f;
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return kFit(kPopoverViewCellHeight);
}

static NSString *kPopoverCellIdentifier = @"kPopoverCellIdentifier";
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    SMKSFPopoverViewCell *cell = [tableView dequeueReusableCellWithIdentifier:kPopoverCellIdentifier];
    if (!cell) {
        cell = [[SMKSFPopoverViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kPopoverCellIdentifier];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    SMKSFPopoverAction *action = _actions[indexPath.row];
