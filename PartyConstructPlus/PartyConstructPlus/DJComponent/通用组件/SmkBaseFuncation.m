...
#import "SMKSecurity.h"
#import "SMKNetConnect.h"
#import <CommonCrypto/CommonDigest.h>

@implementation SmkBaseFuncation
...
+(NSDictionary *)xmlParse:(NSData *)data
{
    ...
    xmlString = [xmlString stringByReplacingOccurrencesOfString:@"encoding=\"GBK\"" withString:@"encoding=\"UTF-8\""];
    xmlString = [xmlString stringByReplacingOccurrencesOfString:@"encoding=\"gbk\"" withString:@"encoding=\"UTF-8\""];
...
+ (NSDictionary *)paramsDecrypt:(NSString *)str
{
    if (str!=nil) {
        ...
        NSString *enStr = [SMKSecurity aesDecrypt:str key:kHOST_AES_KEY vector:kHOST_AES_VECTORKEY];
...
+ (NSDictionary *)paramsEncrypt:(NSDictionary *)dic
{
    if (dic!=nil) {
        ...
        NSString *jsonStr = [NSString stringWithFormat:@"%@",[SMKNetConnect dataTOjsonString:dic]];

        NSString *enStr = [SMKSecurity aesEncrypt:jsonStr key:kHOST_AES_KEY vector:kHOST_AES_VECTORKEY];
